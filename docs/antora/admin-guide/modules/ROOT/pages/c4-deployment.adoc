...
В данном разделе представлены C4-диаграммы развёртывания для трёх моделей: SaaS (облако), On-Premise (enterprise) и локальная разработка.

== SaaS: Cloud Deployment

Диаграмма развёртывания VEDO Core в облачной инфраструктуре (Yandex Cloud / AWS).
...
C4Deployment
    ...
    Deployment_Node(cloud, "Cloud Provider", "Yandex Cloud / AWS") {
        ...
        Deployment_Node(db, "Data Layer") {
            ...
            ContainerDb(immutableBackup, "Immutable Backup Storage", "S3/MinIO + WORM", "Immutable backup")
        ...
        Deployment_Node(admin, "Admin Workstation / Server") {
            Container(cli, "vedo-cli", "Go", "Single binary: linux/amd64, linux/arm64, darwin/amd64, darwin/arm64, windows/amd64")
        }
    }
...
Диаграмма развёртывания VEDO Core в изолированном ЦОД клиента.
...
C4Deployment
    ...
    Deployment_Node(dc, "Data Center", "Isolated network") {
        ...
        Deployment_Node(admin, "Admin Workstation / Server") {
            Container(cli, "vedo-cli", "Go", "Single binary")
        }
    }
...
Диаграмма развёртывания VEDO Core на рабочей станции разработчика.
...
C4Deployment
    ...
    Deployment_Node(workstation, "Developer Workstation", "macOS / Linux / Windows") {
        ...
        Deployment_Node(support, "Local Support Services") {
            ...
            Container(cli, "vedo-cli", "Go", "Local build, debug enabled")
        }
    }
...
|===
| Компонент | Local Dev | Production
...
| Single, no replication
| Primary + 2 standby

| Support DB
| Separate local instance
| Separate cluster, Multi-AZ
...
| Immutable Backup
| Local MinIO WORM (port 9002)
| S3/MinIO + Object Lock, изолированные credentials

| Vault/KMS
| ~/.vedo/config.yaml или VEDO_CLI_TOKEN env
| HashiCorp Vault / encFS

| Auth
| Embedded xref:glossary.adoc#keycloak[Keycloak]
| External Keycloak
...
| Backup
| Manual via `vedo-cli backup`
| Automated 3-2-1 with `vedo-cli backup/verify`

| vedo-cli
| Local build, debug enabled
| Single binary, admin workstation
