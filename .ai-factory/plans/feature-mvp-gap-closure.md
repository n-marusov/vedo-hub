...
- [x] **F1. F11.1 Public landing page + demo showcase (design first)**
  ...
  - **Note (design-first):** `design/pages/landing.pen` created (2026-08-02) as a valid Pencil file following the project format (JSON, `B:` ui-kit import, `$B:` variable refs): hero (eyebrow/title/subtitle), email CTA (Input/SM `B:SX8Ko` + Button `B:3CI7c`), 3 audience cards (Card `B:Nie8I`), 5 demo cards (Продукт/Организация/Процесс/Глоссарий/Событие). Referential-integrity audit PASSED: 0 broken `B:` refs; all descendant overrides (`VtfUp`/`wMRLg`/`esH0z`/`Ap3op`/`kaoPr`/`0ndYv`) resolve in ui-kit.lib.pen.
  - **Acceptance:**
    ...
    - [x] `landing.pen` created with no dangling `B:<id>` refs (referential-integrity audit via `.ai-factory/scripts/_analyze_refs.js` — PASS, 0 broken)
