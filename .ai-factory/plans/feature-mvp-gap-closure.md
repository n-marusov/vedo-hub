...
- [x] **F1. F11.1 Public landing page + demo showcase (design first)**
  ...
  - **Note (design-first):** `design/pages/landing.pen` created and aligned to ADR-DES.UI.public-landing-architecture: 1920×1080, content max-width 1200px, frame ids `landingHeader` (nav h=56px bg=--card, Войти outline), `landingHero` (h1 «VEDO Hub — GitHub + Hugging Face для онтологий» 32px primary, 3 subtitle lines, CTA Создать аккаунт primary + Смотреть демо outline), `landingDemoCards` (h2 «Попробуйте прямо сейчас — без регистрации», 5× Molecule/DemoProjectCard `B:ze4Nt` with class/property stats), `landingRoles` (h2 «Для кого VEDO Hub», 5 role cards: Бизнес-аналитик, Учёный, Онтолог, Разработчик, DevOps), `landingTrust` (4 metrics 45с→<1с / 2ч→10мин / 40-80ч→30мин / 12ч→2мин + open-core banner), `landingFooter` (5 columns Продукт/Ресурсы/Документация/Компания/Сообщество + © 2026). Referential-integrity audit PASSED: 0 broken refs; all descendant overrides resolve.
  - **Acceptance:**
