...
- [ ] **M5: MVP Scope Gap Closure** — Close the remaining gaps between implemented baseline capabilities and the final MVP scope from `specs/vision.md` section 2.5.
  ...
  - [x] **REST API & Auth** — E2E-verified: JWT acceptance (OWNER/VIEWER/EDITOR), 401/403/404 error handling, class/property/individual list, Turtle export, OpenAPI served. Keycloak RBAC gates pass.
  ...
  - [x] **Basic Semantic Diff** — Code complete: entity-level diff (classes, properties, individuals) with summary counts. API: `GET /api/v1/versioning/commits/{id}/semantic-diff`. Unit tests pass.
  ...
  - [x] **Versioning (commit/history/branches)** — E2E-verified: commit history and branch list endpoints reachable.
...
- **Semantic Diff boundary:** Basic Semantic Diff is implemented — entity-level diff (classes, properties, individuals) with summary counts via `GET /api/v1/versioning/commits/{id}/semantic-diff`. Visual graph diff, reasoning-aware diff, manual conflict resolution, and Merge/Pull Request review workflows are post-MVP.
