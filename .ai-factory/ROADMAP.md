...
- [ ] **M7: MVP Acceptance, Security & Demo Readiness** — Stabilize the minimal end-to-end demo chain: group → project → members → template/document → ontology generation → TBox/ABox edit → class/TBox/ABox navigation → commit/history/diff → REST/MCP. Add authorization regression checks for group/project roles, destructive-action safety, empty/error/loading states, smoke tests, demo seed data, and concise operator runbook.
...
- [ ] **M9: Document AI 1.0** — Extend document-to-ontology beyond MVP: OCR for scanned PDFs, batch document upload, XML/CSV support, custom extraction prompts, advanced deduplication, AI-assisted completion, relationship hints, and quality review of generated ontology fragments.
...
- [ ] **M15: Scale, Ecosystem & Marketplace** — Large-graph visualization with WebGL/virtualization, advanced ontology analytics, dedicated enterprise APIs/SLA, GitHub/GitLab sync plugins, custom template marketplace, fork/versioning APIs, public profiles, stars/forks/issues, sponsoring, and partner commissions.
...
- **MVP source of truth:** MVP scope is governed by `specs/vision.md` section 2.5. The MVP is intentionally limited to the demonstrable chain: group → project → members → template/document → ontology generation → TBox/ABox editing → class/TBox/ABox graph navigation → commit/history/basic Semantic Diff → REST API/MCP.
...
- **Document AI priority:** Document-to-ontology generation is a high-priority MVP capability because the target user often starts from real `DOCX`, text `PDF`, `XLSX`, `JSON`, `MD`, or `TXT` artifacts rather than from hand-written OWL/RDF. MVP excludes batch uploads, OCR, custom prompts, and template-as-document-context.
- **Templates in MVP:** Preset templates are included only as onboarding accelerators for the editor. Template marketplace, custom templates, template editor, and template versioning are post-MVP.
...
- **Import/export boundary:** OWL/RDF import/export is deliberately deferred after MVP. The MVP focuses on templates and document generation as the primary onboarding paths.
