...
- [ ] **M5: MVP Scope Gap Closure** — Close the remaining gaps between implemented baseline capabilities and the final MVP scope from `specs/vision.md` section 2.5.
  ...
  - [x] **REST API & Auth** — E2E-verified: JWT acceptance (OWNER/VIEWER/EDITOR), 401/403/404 error handling, class/property/individual list, Turtle export, OpenAPI served. Keycloak RBAC gates pass. API guardrails added: deprecation headers + 501/x-vedo-status:planned stubs per ADR-DES.API.rest-gitlab-alignment.
...
- [ ] **M11: Publishing, Import/Export & Public Browse 1.0** — Implement deferred exchange and publication capabilities: OWL/RDF import/export, XLSX/DOCX exchange, snapshot publishing pipeline with CQRS serving layer (separate read-only store), binary visibility model (public/restricted via API key per ADR-DES.INFRA.publishing-extension), maintainer gate for publish action, releases naming, public read-only browse API/UI, public ontology visibility model, public search, publication metadata, DOI-ready metadata.
