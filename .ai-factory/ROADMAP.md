...
- [ ] **M5: MVP Scope Gap Closure** — Close the remaining gaps between implemented baseline capabilities and the final MVP scope from `specs/vision.md` section 2.5.
  - [~] **F13.1 Forks** — Code merged (`d823d8e`): `POST /api/v1/projects/{id}/fork`, Fork Saga, 5 demo projects, UI. ⚠ Security tests (`tests/security/fork_bola_test.go`) still in RED phase — all 7 tests skipped with `t.Skip("not implemented yet")`. Activation to GREEN pending.
  - [x] **Group/project/member CRUD** — E2E-verified (`test-output2.txt`): create, list, get, update, delete groups; create, list, get, update, delete projects; add/update/remove members with roles. 64/64 API tests pass.
  ...
  - [ ] **REST API architectural alignment** — Formalize 3 foundational ADRs (write-path invariant, REST GitLab alignment, publishing extension) and update glossary, vision, ROADMAP, traceability. Add API guardrails: deprecation headers on /api/v1/ontologies/*, 501 stubs for /releases, /merge_requests, /protected_branches. Fix grpcPool.Close() lifecycle bug. Pre-M5 prerequisite for M10 (invariant enforcement) and M11 (publishing pipeline).
  - [x] **Basic SPARQL & CYPHER** — E2E-verified: SELECT/MATCH execute, mutations rejected, invalid syntax → 400, unauthenticated → 401. `QUERY_TIMEOUT=30s`, parameterized `LIMIT` in code.
  - [x] **Basic Semantic Diff** — Code complete: entity-level diff (classes, properties, individuals) with summary counts. API: `GET /api/v1/projects/{pid}/repository/commits/{sha}/diff`. Unit tests pass.
  - [x] **Commenting backend** — Code complete: CRUD comments, reply threading, `/comment-feed` endpoint, PostgreSQL-backed with integration tests.
  - [ ] **Versioning (commit/history/branches)** — E2E-verified: commit history and branch list endpoints reachable.
  - [ ] **Basic merge blocking on conflicts** — Per vision 2.5 (F3.828): «если конфликт есть — merge блокируется с понятным сообщением». Current code reports `conflict_count` but does NOT block merge. Add guard: `409 MERGE-CONFLICT`.
  - [ ] **TBox editor polish** — Per vision 2.5 (F2.739-772): CRUD classes/properties/annotations, hierarchy drag-n-drop, search with autocomplete. GUI e2e tests exist (`tests/e2e/specs/gui/flows/editor.spec.ts`) — test run pending.
  - [ ] **Minimal ABox CRUD** — Per vision 2.5 (F4.774-787): create/read/update/delete individuals, list by class, search by id/label.
  - [ ] **Class Hierarchy / TBox Graph / ABox Graph views** — Per vision 2.5 (F1.789-812): three separate navigation views. GUI e2e tests exist (`graph-visualization.spec.ts`, `browse.spec.ts`) — test run pending.
  - [ ] **Comment frontend wiring** — Wire frontend feed/reply UI to commenting-service API. E2E test exists (`tests/e2e/specs/gui/flows/commenting-flow.spec.ts`) — test run pending.
  - [ ] **SPARQL audit logging** — Per vision 2.5 (F3.878): audit trail for read-only SPARQL (trace_id, user, query, LIMIT, execution time).
  - [ ] **F11.1 Public landing page + demo showcase** — Public hero page (no auth): positioning statement, email CTA, role-based audience sections, clickable demo ontologies (Vue Flow graph via GraphQL, served in publish-browse-ui). VEDO Demos visible without auth. Design first: landing.pen in design/pages/. Part of F11 Publishing — minimal showcase, not full Public Browse 1.0 (M11).
...
- **E2E test evidence:** API E2E tests (64 specs, `tests/e2e/specs/api/`) pass when JWT acceptance is configured correctly (`test-output2.txt`: 64 passed, 8.3s). Group/Project/Member CRUD, REST API, SPARQL/CYPHER, GraphQL, Auth gates, and Versioning endpoints are covered. GUI smoke/flows E2E specs exist (`tests/e2e/specs/gui/`) — test run pending. Fork security tests (`tests/security/fork_bola_test.go`) still in RED phase: all 7 tests skipped with `t.Skip("not implemented yet")` — needs GREEN activation.
