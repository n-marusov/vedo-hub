	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if !knownPath {
			writeJSON(w, http.StatusNotFound, map[string]any{
				...
				"message":   "The requested endpoint " + r.URL.Path + " does not exist",
				"available": []string{"/", "/health", "/ready", "/metrics"},
			})
		...
		if r.Method != http.MethodGet {
			writeJSON(w, http.StatusMethodNotAllowed, map[string]any{
				...
				"message":   "Method " + r.Method + " not allowed on " + r.URL.Path,
				"available": []string{"/", "/health", "/ready", "/metrics"},
			})
		...
		switch r.URL.Path {
		case "/":
			writeJSON(w, http.StatusOK, map[string]any{"name": serviceName, "version": "0.2.0", "description": "API Gateway for VEDO Core", "stub": false})
