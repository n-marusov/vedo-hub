...
export function useBatchUpload() {
  ...
  const batchResult = computed<BatchUploadResult>(() => ({
    ...
    previews: files.value
      ...
      .map((f) => f.preview as ExtractionPreview),
    failed: files.value
  ...
  function resolveConflict(
    ...
    if (resolution === 'keep_a') {
      ...
      const stepInMerge = mergedSteps.value.find((s) => s.id === conflict.stepA.id)
      if (stepInMerge) {
  ...
  function findStep(entityId: string, sourceFile: string): SequenceStep | undefined {
    for (const fileState of files.value) {
