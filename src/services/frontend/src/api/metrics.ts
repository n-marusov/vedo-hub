...
export async function getOntologyMetrics(ontologyId: string): Promise<OntologyMetrics> {
  console.info(JSON.stringify({
    level: "info", msg: "metrics.ontology.get.request",
  ...
  try {
    ...
    console.info(JSON.stringify({
      level: "info", msg: "metrics.ontology.get.success",
      ontologyId, ts: new Date().toISOString(),
    }));
    return data;
  } catch (err: any) {
    const msg = err.response?.data?.error?.message ?? err.message ?? "Failed to get ontology metrics";
    console.error(JSON.stringify({
      level: "error", msg: "metrics.ontology.get.failed",
...
export async function listAllMetrics(): Promise<OntologyMetrics[]> {
  console.info(JSON.stringify({
    level: "info", msg: "metrics.list.request",
  ...
  try {
    ...
    console.info(JSON.stringify({
      level: "info", msg: "metrics.list.success",
      count: data?.length ?? 0, ts: new Date().toISOString(),
    }));
    return data ?? [];
  } catch (err: any) {
    const msg = err.response?.data?.error?.message ?? err.message ?? "Failed to list metrics";
    console.error(JSON.stringify({
      level: "error", msg: "metrics.list.failed",
