...
function parseErrors(output) {
  ...
  while ((match = regex.exec(output)) !== null) {
    errors.push({ file: match[1], line: Number(match[2]), code: match[3], message: match[4] })
  }
  return errors
}
...
function applyFixes(errors) {
  ...
  for (const err of errors) {
    ...
    if (err.code === '6133') {
      const varName = err.message.match(/"([^"]+)"/)?.[1] || err.message.match(/'([^']+)'/)?.[1]
      if (varName?.startsWith('_')) {
        const clean = varName.slice(1)
        if (content.includes(`{{ ${clean}`) || content.includes(`:${clean}`) || content.includes(`v-for="${clean}`) || content.includes(`v-if="${clean}`) || content.includes(`v-model="${clean}`) || content.includes(`v-show="${clean}`) || content.includes(`v-text="${clean}`) || content.includes(`v-html="${clean}`)) {
          const fixes = fixesByFile.get(fileKey) || []
          fixes.push({ line: err.line, old: `const ${varName}`, new: `const ${clean}` })
    ...
    if (err.code === '2339') {
      const propName = err.message.match(/"([^"]+)"/)?.[1] || err.message.match(/'([^']+)'/)?.[1]
      if (propName) {
        for (let i = 0; i < lines.length; i++) {
          if (lines[i].includes(`const _${propName}`) || lines[i].includes(`const _${propName} =`)) {
            const fixes = fixesByFile.get(fileKey) || []
            fixes.push({ line: i + 1, old: `const _${propName}`, new: `const ${propName}` })
            fixesByFile.set(fileKey, fixes)
            break
          }
        }
    ...
    if (err.code === '2345' && (err.message.includes('displayName') || err.message.includes('Avatar'))) {
      const fixes = fixesByFile.get(fileKey) || []
      if (line.includes(':name=')) fixes.push({ line: err.line, old: ':name=', new: ':displayName=' })
      fixesByFile.set(fileKey, fixes)
    }
    ...
    if (err.code === '2345' && err.message.includes('Badge')) {
      const fixes = fixesByFile.get(fileKey) || []
      if (line.includes(':label=')) fixes.push({ line: err.line, old: ':label=', new: ':text=' })
      fixesByFile.set(fileKey, fixes)
    }

    // TS2322: null → undefined for string | undefined
    if (err.code === '2322' && err.message.includes('null') && err.message.includes('undefined')) {
      const m = line.match(/ref<[^>]*null[^>]*>\([^)]*null[^)]*\)/)
      if (m) {
        const fixes = fixesByFile.get(fileKey) || []
        fixes.push({ line: err.line, old: m[0], new: m[0].replace(/null/g, 'undefined').replace(/string \| null/, 'string | undefined') })
        fixesByFile.set(fileKey, fixes)
      }
    }
    ...
    if (err.code === '2322' && err.message.includes('provider')) {
      ...
      if (m) {
        const fixes = fixesByFile.get(fileKey) || []
        fixes.push({ line: err.line, old: `id: '${m[1]}'`, new: `id: '${m[1]}' as const` })
        fixesByFile.set(fileKey, fixes)
      }
    }

    // TS2345: string | null → string | undefined
    if (err.code === '2345' && err.message.includes('null') && err.message.includes('undefined')) {
      const m = line.match(/ref<string \| null>\(null\)/)
      if (m) {
        const fixes = fixesByFile.get(fileKey) || []
        fixes.push({ line: err.line, old: m[0], new: 'ref<string | undefined>(undefined)' })
        fixesByFile.set(fileKey, fixes)
      }
    }
  }
...
async function main() {
  ...
  while (iteration < maxIterations) {
    ...
    if (fixed === 0) {
      ...
      for (const e of errors.slice(0, 10)) console.log(`  ${e.file}:${e.line} TS${e.code}: ${e.message.slice(0, 120)}`)
