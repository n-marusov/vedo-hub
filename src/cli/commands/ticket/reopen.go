...
func ExecuteReopen(input *TicketCliInput) *TicketCliOutput {
	if input.TicketID == "" {
	...
	ticket, err := defaultStore.SetStatus(input.TicketID, "reopened")
	if err != nil {
	...
	slog.Info("ticket.reopen.success",
		"ticket_id", input.TicketID,
		"actor", input.Actor,
	)

	return successOutput(ticket, "Ticket reopened successfully", audit)
}
