...
// Preload the English locale so component tests render real translated text
// (t() returns raw keys when no locale messages are loaded).
// NOTE: relative import — vitest.setup.ts is outside src/, so the @ alias
// is not resolved here by the TS config.
