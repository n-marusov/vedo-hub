...
vi.mock('../api/extraction', () => ({
  batchUploadDocuments: vi.fn().mockImplementation(async (opts: { files: File[] }) => {
...
describe('useBatchUpload', () => {
  ...
  describe('Initial state', () => {
    ...
    it('has zero counts initially', () => {
      const { pendingCount, uploadingCount, successCount, failedCount, totalCount } =
        useBatchUpload()
  ...
  describe('Upload all files', () => {
    it('uploads all pending files', async () => {
      const { files, addFiles, uploadAll, allDone, successCount, mergedSteps } = useBatchUpload()
    ...
    it('handles partial failure', async () => {
      ...
      const { files, addFiles, uploadAll, hasFailedFiles, successCount, failedCount } =
        useBatchUpload()
      ...
      // good.md succeeded
      expect(files.value.find((f) => f.fileName === 'good.md')?.status).toBe('success')
      // fail.txt failed
      expect(files.value.find((f) => f.fileName === 'fail.txt')?.status).toBe('error')
    })
  })
