...
describe('ForkDemoDialog', () => {
  ...
  it('should render list of 5 demo projects with names and descriptions', async () => {
    ...
    const comp = (await import('@/components/projects/ForkDemoDialog.vue')).default
  ...
  it('should show empty state when no demo projects available', async () => {
    ...
    expect(document.body.textContent || '').toContain('No demo projects available')
  })
  ...
  it('should close dialog when Cancel button is clicked', async () => {
    ...
    if (cancelBtn) {
      ...
      const closeEvent = emitted['update:modelValue'] || emitted.close || emitted.cancel
