...
export async function listCommits(
	...
	try {
		...
		const msg =
			e.response?.data?.error?.message ?? e.message ?? "Failed to list commits";
		console.error(
			JSON.stringify({
				level: "error",
...
export async function getBranch(branchId: string): Promise<BranchInfo> {
	...
	try {
		...
		const msg =
			e.response?.data?.error?.message ?? e.message ?? "Failed to get branch";
		console.error(
			JSON.stringify({
				level: "error",
...
export async function listTags(ontologyId: string): Promise<TagInfo[]> {
	...
	try {
		const { data } = await api.get(
			`/versioning/tags?ontology_id=${ontologyId}`,
		);
		console.info(
			JSON.stringify({
				level: "info",
		...
		const msg =
			e.response?.data?.error?.message ?? e.message ?? "Failed to list tags";
		console.error(
			JSON.stringify({
				level: "error",
