#!/bin/bash
# @ctx: Contract gate runner — native stub and milestone 003 contract checks
set -euo pipefail
...
if [ -n "$GO_CMD" ]; then
  ...
  # @ctx: each Go module has its own go.mod — run from module root
  ...
  # @ctx: ticket-api integration tests (separate Go module)
...
# @ctx: existing milestone 001/002 contract tests
